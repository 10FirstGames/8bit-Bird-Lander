...
function feedBack(eventName, string, arg)
	if active == false then
		thisMessage = display.newText("",  w + w/4, h/2 + howFarDown, "ArcadeClassic", 78 )
	...
	elseif eventName == "Landed" then
		...
		otherMessage = display.newText("",  w + w/4, h/2 + howFarDown, "ArcadeClassic", 78 )
...
playerObject = display.newImage("bird1.png",30, -100)
--playerObject:setFillColor(255,0,0)
--playerObject:setStrokeColor(0, 0, 0) 
playerObject.strokeWidth = 1
physics.addBody(playerObject, { density = 4.0, friction = 1.0, bounce = 0 } )

platform1 = display.newImage("rock.png", 20,200)
...
platform2 = display.newImage("rock.png", 60,200)
...
nest = display.newImage("nest.png", 260,190)
...
playerObject = display.newImage("bird2.png", 30, -100)
--playerObject:setFillColor(255,0,0)
--playerObject:setStrokeColor(0, 0, 0) 
playerObject.strokeWidth = 1
physics.addBody(playerObject, { density = 4.0, friction = 1.0, bounce = 0 } )

platform1 = display.newImage("rock.png", 20,100)
--platform1:setFillColor(50, 50, 50) 
physics.addBody( platform1, "kinematic", { friction=0.7, bounce = 0.1 } )
platform1.isPlatform = true -- custom flag, used in drag function

nest = display.newImage("nest.png", 230,60)
physics.addBody( nest, "kinematic", { friction=0.7 } )
--nest:setFillColor(90,50,20,255)
nest.isPlatform = true
nest.myName = "theNest"
...
playerObject = display.newImage("bird3.png", 30, -100)
--playerObject:setFillColor(255,0,0)
playerObject:setStrokeColor(0, 0, 0) 
playerObject.strokeWidth = 1
physics.addBody(playerObject, { density = 5.0, friction = 1.0, bounce = 0 } )

platform1 = display.newImage("rock.png", 160,60)
--platform1:setFillColor(50, 50, 50) 
physics.addBody( platform1, "kinematic", { friction=0.7, bounce = 0.1 } )
platform1.isPlatform = true -- custom flag, used in drag function

platform2 = display.newImage("rock.png", 200,90)
--platform2:setFillColor(50, 50, 50) 
physics.addBody( platform2, "kinematic", { friction=0.7, bounce = 0.1 } )
platform2.isPlatform = true -- custom flag, used in drag function

nest = display.newImage("nest.png", 160,120)
physics.addBody( nest, "kinematic", { friction=0.7 } )
--nest:setFillColor(90,50,20,255)
nest.isPlatform = true
nest.myName = "theNest"
...
platform1 = display.newImage("rock.png", 10,80)
--platform1:setFillColor(50, 50, 50) 
physics.addBody( platform1, "kinematic", { friction=0.7, bounce = 0.1 } )
platform1.isPlatform = true -- custom flag, used in drag function

platform2 = display.newImage("rock.png", 50,110)
--platform2:setFillColor(50, 50, 50) 
physics.addBody( platform2, "kinematic", { friction=0.7, bounce = 0.1 } )
platform2.isPlatform = true -- custom flag, used in drag function

nest = display.newImage("nest.png", 10,140)
physics.addBody( nest, "kinematic", { friction=0.7 } )
--nest:setFillColor(90,50,20,255)
nest.isPlatform = true
nest.myName = "theNest"
...
function clearDistMarkerY()
	display.remove(yDistanceRect)
	display.remove(yDistancePlayerMarkerRect)
	display.remove(distLineY)
...
function getDistanceSoar()
	...
	yDist = landedAtY - lastFlapY --this is how we get the distance traveled from last flap to landing for X
	print("")
	print("")
	print(yDist)
	yDist = yDist/10 -- this turns the xdist in to "FEET" 
	print(yDist)
	howManyFeetY = math.floor (yDist) -- this rounds the number of feet down. 
	print(howManyFeetY)
	-- if yDist < 0 then
	-- 	yDist = yDist * -1 -- if the distance is negative (higher) then make it a positive number. 
	-- end
	if howManyFeetY < -5 then -- this checks to make sure the distance is under the landing plane by 5 feet. 
		print("distance greater than 2 so I will try to draw some lines")
		yDistanceRect = display.newRect(nest.x-40 ,lastFlapY, 10, 2) -- this draws the last flap mark
		yDistancePlayerMarkerRect = display.newRect(nest.x - 40,playerObject.y, 10, 2) -- this draws the land at mark
		distLineY = display.newLine(yDistanceRect.x,yDistanceRect.y, yDistancePlayerMarkerRect.x,yDistancePlayerMarkerRect.y ) -- this draws the line between the two
		
		gameLevel:insert(yDistanceRect) --  puts the last flap mark in the game level group
		gameLevel:insert(yDistancePlayerMarkerRect) --  puts the landed mark in the game level group
		gameLevel:insert(distLineY) --  puts the line in the game level group
		timer.performWithDelay(2000, clearDistMarkerY) -- calls the function that gets rid of the lines and marks.

		howManyFeetY = howManyFeetY * -1

		feedBack("longShot", "From " .. howManyFeetY .. " feet below the nest!", howManyFeet) -- sends message to feedBack
	end

	if screenLap == 0 then -- if you have not come from off the screen
		xDist = landedAtX - lastFlapX --this is how we get the distance traveled from last flap to landing for X
		xDist = xDist/10 -- this turns the xdist in to "FEET" 
		howManyFeet = math.floor (xDist) -- this rounds the number of feet down. 
		if xDist < 0 then
			xDist = xDist * -1 -- if the distance is negative then make it a positive number. 
		end
		if howManyFeet > 15 then -- this checks to make sure the distance is over 15 feet away. 
			xDistanceRect = display.newRect(lastFlapX,playerObject.y - 25, 2, 10) -- this draws the last flap mark
			xDistancePlayerMarkerRect = display.newRect(playerObject.x,playerObject.y - 25, 2, 10) -- this draws the land at mark
			distLine = display.newLine(xDistanceRect.x,xDistanceRect.y, xDistancePlayerMarkerRect.x,xDistancePlayerMarkerRect.y ) -- this draws the line between the two
			
			gameLevel:insert(xDistanceRect) --  puts the last flap mark in the game level group
			gameLevel:insert(xDistancePlayerMarkerRect) --  puts the landed mark in the game level group
			gameLevel:insert(distLine) --  puts the line in the game level group
			timer.performWithDelay(2000, clearDistMarkerX) -- calls the function that gets rid of the lines and marks.

			feedBack("longShot", "From " .. howManyFeet .. " feet away!", howManyFeet) -- sends message to feedBack
		end
...
function setUpControlls(playWithOneHand)
	...
	controlBg = display.newImage( "base1.png", 0, h/2)
...
function levelPickerFunction()
	...
	by = -20
	--draw BackGround
	levelPickerBG = display.newRect( -40, -40, w, h/2)
	levelPickerBG:setFillColor(0,0,0, 200)
...
function endLevel()
	if nextLevelNow == true then
		...
		print("At one point you fell to death from " .. fallToDeathFrom .. " feet high")   -- A height you died from.
		print("In this last leverl you landed from a height of " .. landedFromHeight .. " feet.") -- A height you landed from.
		clearLevelTimer()
		if upFlaps < 1 then
...
-- -- speed at landing (the speed of the last NOT MISS)	                                               -- done

-- Distance of player from nest on last player action Y (we already have the x)                        -- done
-- height of player object             --This was done with topHeight and curHeight --				   -- done
-- -- height of fall to death 
-- -- height of fall to water. 
-- -- height of fall to landing. 

-- add a way to change how long it takes for feedBack to move accross the screen in to the funciton call.
...
---- BUGS ------------------------------------------------------------------------------------------------
-- sometimes when you land on the nest (in the first level it was noticed more than once) nothing regesters.
